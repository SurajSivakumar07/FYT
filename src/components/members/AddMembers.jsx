...
// //   //   const priceAfterDiscount = Math.max(0, planPrice - disc); // Ensure total isn't negative
// //   //   return Math.floor(Math.max(0, priceAfterDiscount - paid));
// //   // };
...
//     const discountedPrice = planPrice * (1 - discountPercentage / 100);
//     return Math.floor(Math.max(0, discountedPrice - paid));
//   };

//   const calculateTotalAmount = (planPrice, discount) => {
//     const discountPercentage = parseFloat(discount) || 0;
//     return (planPrice * (1 - discountPercentage / 100)).toFixed(2);
//   };
...
//         parseFloat(memberData.transaction.discount) > 100)
//     ) {
...
//         balance: parseInt(memberData.transaction.balance) || 0,
//         payment_date: convertToISODate(memberData.transaction.payment_date),
//         discount: parseFloat(memberData.transaction.discount) || 0,
//       },
//       photo_url_1: memberData.photo_url_1,
//       photo_url_2: memberData.photo_url_2,
//     };
...
//                               plans.find(
//                                 (plan) =>
//                                   plan.id === memberData.membership_plan_id
//                               ).price
//                             ),
//                             memberData.transaction.discount
//                           )
...
export default function AddMembers() {
  ...
  const [memberData, setMemberData] = useState({
    ...
    transaction: {
      ...
      status: "paid",
      balance: 0,
      discount: 0, // Discount as flat amount
    },
    photo_url_1: null,
    photo_url_2: null,
  });
  ...
  const calculateBalance = (planPrice, amountPaid, discount) => {
    ...
    const totalAfterDiscount = planPrice - discountAmount;
    return Math.floor(Math.max(0, totalAfterDiscount - paid));
  };

  const calculateTotalAmount = (planPrice, discount) => {
    const discountAmount = parseFloat(discount) || 0;
    return (planPrice - discountAmount).toFixed(2);
  };
  ...
  const validateForm = () => {
    ...
    if (memberData.membership_plan_id) {
      const selectedPlan = plans.find(
        (plan) => plan.id === memberData.membership_plan_id
      );
      if (
        selectedPlan &&
        parseFloat(memberData.transaction.discount) >
          parseFloat(selectedPlan.price)
      ) {
